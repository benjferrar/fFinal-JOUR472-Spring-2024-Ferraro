...
**Q1.** How many departments have offered the courses with "Climate" in the title and at least one seat? Which department has offered the most? **26 departments; Atmospheric and Ocean Sciencw offers the most**
...
**Q2.** Of the departments listed in A2, which one is the most interesting to you, and why? **Anthropology is the most interesting to me, specifically with the amount. I don't usually associate climate science with that subject,**
...
**A3. Business and Management, 125 departments, The departments differ in each result by syllabi posted versus just the most courses in the semester**
...
maryland_expenses_with_state <- maryland_expenses |>
  mutate(
    state = case_when(
        str_detect(address, " in-state ") ~ "IN",
        str_detect(address, " out-of-state ") ~ "OUT",
        .default = NA
      )
  )
```

**Q6.** Choose your own adventure: using either the UMD courses or campaign expenses data, ask and answer a question that is different than the previous ones. You can use one of the ones you provided in the pre_lab. **A6: Can we find the total amount spent each week by Wes Moore's committee? The week of June 12th, 2022 seems to be the most expensive week for costs.**

```{r}
maryland_expenses |>
  mutate(expenditure_date_new = mdy(expenditure_date)) |>
  mutate(week = floor_date(expenditure_date_new,"week")) |> 
  filter(committee_name== 'Moore  Wes For Maryland') |> 
  group_by(week) |> 
  summarize(total_amount = sum(amount))
