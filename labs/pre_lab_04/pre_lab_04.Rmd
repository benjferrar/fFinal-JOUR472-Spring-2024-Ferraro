...
**Task** Run the following code to group and count records in the Grantee column, and filter to examine only recipients that start with Y. Name a problem that you see that is preventing proper grouping. **Some grantee names are very similar, with minor differences such as "of" and "Of."**
...
Run the following code to make a copy of the Grantee field called grantee_clean. Store it as a new object called cleaned_md_grant_loans Click the black arrow on the table to examine the Grantee and grantee_clean columns. Looking at the results on the first 3 pages only (first 30 rows), name two changes the refinr package made, by comparing Grantee to grantee_clean. **The column was filtered to see if names were similar enough to be one thing; then the similar names combined into one indentifier to make it easier to spot similar grantees.**
...
**Task** Run the following code to group by grantee_clean and Grantee, then filter for recipients that start with Y. Name three changes the refinr function made in the first 30 results. **We can count by Grantee clean, we can sort by Y receipients, and all the "Ofs" are capitalized and sorted correctly.**
...
Run the following code to use the n_gram_merge() function to clean the data, then group by grantee_clean and Grantee to examine recipients that start with Y. Look at the differences. Does the changes it made vary substantially from the previous use of key_collision_merge **The methods don't vary too substantially.**
