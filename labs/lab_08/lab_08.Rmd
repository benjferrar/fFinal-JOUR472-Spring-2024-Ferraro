...
**December had the greatest percentage of calls (around 10%).**
...
pg_calls_by_month <- pg_calls |>
  ...
  group_by(month) |> 
  summarize(pct_total = n()/1397*100) |> 
...
pg_calls_by_month |>
  ...
  coord_flip() +
  labs(
    title = 'One in Ten Overdose Calls in Prince George\'s County happens in December',
    x = 'month',
...
**I think this chart analyses the median income for each zip code, and translate the codes to towns or cities in order to report the data out. It seems the general trend is that the higher the income, the total calls decreases.**

```{r}

pg_calls_zipcodes <- pg_calls |> group_by(zipcode) |>
  summarize(total_calls = n())

md_median_income_zip <-get_acs(geography = 'zcta', variables= 'B19013_001', state = 'MD', year=2019)

pg_calls_with_income <- pg_calls_zipcodes |> inner_join (md_median_income_zip, by = c('zipcode'='GEOID'))

ggplot(pg_calls_with_income, aes(x=estimate, y=total_calls)) + geom_point() + 
  labs(
    title = 'Wealthier areas of PG County have fewer 911 overdose calls',
    x = 'Median income',
    y = 'Total Calls'
  ) +
  geom_text(aes(x=estimate,y=total_calls,label = zipcode), check_overlap = TRUE, size = 2, angle = 45)
```
