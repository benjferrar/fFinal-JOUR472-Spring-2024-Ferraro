...
##### A1: NAMELSD would be better because it better differentiates Baltimore county and baltimore city in the column.
...
md_childcare_counties <- md_childcare_counties |>
  ...
  coord_flip() +
  labs(
    title="Montgomery County Has The Most Overdue Childcare Costs",
    y = "percentage of costs ",
    x = "county",
...
##### **A3: Garrett County has the biggest rate of children per facility (8,750)**
...
##### **A4**: Eastern Maryland seems to be more lacking in childcare services.
...
md_med_income <- get_acs(geography = 'county', variables='B19013_001', state='MD', year=2022)
```
...
ggplot(md_childcare_income, aes(x=median_income, y=child_rate)) + geom_point() + 
  labs(
    title = 'Median income per Childcare Rate for Maryland counties in 2022',
    x = 'Median income',
    y = 'Child Rate',
    caption= "Source: USA Fact") +
  geom_text(aes(x=median_income,y=child_rate,label = NAME), check_overlap = TRUE, size = 2, angle = 45)
```

```{r}
# Create a correlation test.

cor.test(md_childcare_income$median_income, 
        md_childcare_income$child_rate)
```

##### A5: Our p-value is very high at 0.6067, it is hard to determine statistical significance between income and rate of childcare facilities.
...
##### A6: "Emergency bag," "missing items" and "missing emergency" stand out because it shows that maybe there are issues in childcare services in Maryland.
