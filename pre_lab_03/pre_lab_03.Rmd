...
**Task** Run the following codeblock to reload the data, using every row to set the data types. Does it show any parsing errors when you run? Answer below **Yes, there are delimiter errors and character errors.**
...
**Task** Run the following codeblock to glimpse the data. What data type is the "Vendor Name" field? What data type is the "Amount" field? What data type is the "Date" column? Answer below. **A4: Character, Numeric, Character**
...
**Task** Run the following codeblock to group by the vendor's zip code and show any that are less than five characters in length. How many zip codes do not have five characters? What do you think is happening with this data? **The count of zip codes is being sorted in descending order.**
...
**Task** Run the following codeblock to count the number of payments. Looking at the result, what jumps out at you as potentially problematic? **The presence of N/A in some category rows.**
...
**Task** Run the following code to arrange the grants & loan data so that the smallest amount is first. Describe what you see. **Some grants range from 60 dollars to hundreds of dollars, and are sorted in the data chart from least to greatest.**
...
**Task** Run the following codeblock to use the `clean_names()` function from janitor to standardize column names. How does the function change the headers? Answer below. **Answer**
...
**Task** Run the following codeblock to check for duplicate rows using get_dupes(). How many duplicates are possible? **0**
...
**Task** Run the following codeblock to use distinct() to get rid of duplicate rows. How many rows does the new dataframe have? Answer below. **17,740 rows**
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, creating a new column. Look at the difference in the result - what changed? **Zip code originals and new zip codes are in seperate columns.**
...
**Task** Write and run code to answer our original question: which zip code has gotten the most amount of money from the Maryland Tourism Board? Does that zip code make sense to you? **I thought I could just use the code above and sort totals of descending order, but I guess I am wrong. Am encouraged to see the correct path to do this.**
