...
**Answer We don't want to delete more data, so creating another variable/columns will allow us to store all our data in separate ways.**
...
```{r}
and_theatre_seats_15 <- umd_courses |> filter
(department== "Theatre" | & seats >= 15)

nrow(and_theatre_seats_15)
```

So AND gives us the same answer we got before. What does using the OR operator give us? **41011**
...
**Answer: The column has dashes instead of slashes so the data can work with RStudio better. A new column was created based on the output of the prior function.**
...
**Answer The month column is sorted by descending total amount.**
...
**Answer It's combining the total votes and subsequent percentages related to Moore and total votes.**
...
**It makes sense because the data was sorted by statements so we can see that statistic easier.**
...
1.  **Can we find the percentage of Cox votes from the overall vote?**

2.  **Can we have a list descending for the total votes in each county?**

3.  **Can we find the percentage of classes from the whole in UMD Testudo that are graduate-level?**
