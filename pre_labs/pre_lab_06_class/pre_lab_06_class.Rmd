...
**Task** Load the four CSV population estimate files. Do they have the same column structure? **Yes**
...
**Task** Run the following code to join Maryland population data and the 2022 voter registration data together using the common column county as the key. How many rows are there now? How many *should* there be? **We should get 24 rows.**
...
**Task** Load the Maryland demographic data and examine it. What kind of information does it contain? **Mostly racial data and age information of the people in the data.**
...
**Task** Using mutate, let's find the county with the highest 18+ Black population as a percentage of all population 18 and over and also the percentage of Democrats in that county. Put your answers below. **Prince George's County**
...
**Task** Run the following code to load the Zip Code Tabulation Area data for Maryland. What's the datatype of the ZCTA5N column? **Zip codes**
...
**Task** Run the following code to change the datatype of ZCTA5N from numeric to character. What's the datatype of ZCTA5N? **Is it not still zip codes?**
